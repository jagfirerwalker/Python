...
class RectangularRoom(object):
    ...
    def getRandomPosition(self):
        ...
        """
        randX = random.choice(range(0,self.width))
        randY = random.choice(range(0,self.height))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        move = self.robotPosition.getNewPosition(self.robotDirection,self.speed)
        
        if not self.room.isPositionInRoom(move):
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    def listOfRobots(num_robots, room, speed, robot_type):
       robots = []
    ...
    def simulateClean(room, min_coverage):
        
        cleanPercentage = 0.0
        clean = 0.0
        steps = 0
        robots = listOfRobots(num_robots, room, speed, robot_type)
        total = float(room.getNumTiles())
        
        while cleanPercentage < min_coverage:
            for robot in robots:
            ...
            cleanPercentage = clean / total
        
        return steps
        
    def simulateTrials(room, num_trials):
        totalSteps = 0
        
        for trial in range(num_trials):
            room = RectangularRoom(width, height)
            
            totalSteps += simulateClean(room, min_coverage)
        
        return totalSteps / num_trials
    
    return simulateTrials(room, num_trials)
